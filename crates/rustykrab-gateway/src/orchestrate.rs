...
///
/// The prompt is intentionally minimal (~100 tokens): identity, memory hint,
/// and security policy. Tool definitions are sent via the API's structured
/// `tools` parameter, not duplicated in the prompt.
async fn build_and_inject_system_prompt(
    state: &AppState,
    conv: &mut Conversation,
    _user_content: &str,
) {
    let profile = &state.harness_profile;
...
async fn prepare_agent(
    ...
    .with_config(state.harness_profile.to_agent_config());
